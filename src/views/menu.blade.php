<div class="hidden w-full md:block md:w-auto" id="navbar-default">
    <ul class="flex flex-col mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
      ...
      <li class=" relative group p-4">
        ...
        <ul class=" visited:hidden opacity-0 -z-10 group-hover:z-10 ease-in-out duration-300 absolute min-w-[200px] scale-y-0 group-hover:scale-y-100 mt-1 w-max group-hover:visited:visible group-hover:translate-y-0 group-hover:opacity-100 top-12 dark:bg-gray-800 rounded-b-xl shadow-md">
            @foreach ($item->children as $child )
            <li class="text-white p-3 cursor-pointer hover:bg-gray-700">{{$child->label}}</li>
            @endforeach
        </ul>
